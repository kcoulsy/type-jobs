export type { Driver, DriverConnection, DriverJob, DriverOptions, DriverQueue, DriverWorker } from "./types";
...
export function createDriver<TData = any, TResult = any>(
	...
	switch (config.type) {
		case "redis":
			return new RedisDriver<TData, TResult>(config.redis) as Driver<TData, TResult>;
		case "rabbitmq":
